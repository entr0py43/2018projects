...
My approach:  Work out the relationships between variables using function approximation.  If/when I come across the correct way to do the optimization, I'll update this page.      

Background:  Decided to participate in this 'over the weekend' data science 'hackathon-from-home' type competition. No aspirations of placing, just learning. I agreed to participate casually, with a couple of friends.  We'd all approach the problem individually (stipulated by the rules, it was a 'hiring competition', which is also the reason I cannot share the dataset), and compare notes afterwards.  
  
What I'm presenting here is my interpretation of the question, and approach to the problem.  (Unfortunately, there was some ambiguity as to the actual structure of the problem) 
...
The goal is to maximize Total Net Revenue, defined by:  the updated Probability, P(baseline + incentive bump) *times* the Premium on the policy *minus* whatever you spent on Incentives for that policy.  

### Considerations

* Probability ranges from 0-1.  If you spent on incentives for a policy with a high 'baseline' probability of renewal (pre-incentives), that is probably a poor investment.
...
* The smaller the policy premium, the less you spend on incentives, since the incentive cost per % improvement will provide diminishing returns as the curve flattens out.  The optimium ROI will depend on the premium amount (I'll cover in a minute)

## Scoring Metric
I'll ignore for now, since the dataset isn't public. Your submission included P(baseline) and incentive per policy in the test set.  Scoring was a weighted average that favored the AUC (of the ROC) for your Part A solution (70% of score), and the total net revenue.  
...
auc <- roc(trainCC$renewal, lr0pred); auc
```

So, that's enough for here, I'll make a follow-up post exploring how to optimize that binary classifier model. But since the best AUC I saw was ~.85, I call that darn good for a first-pass.  FWIW, feature engineering did little to enhance the model, according to several of the top-ranked participants, and my own fiddling (quite frustrating!).
...
```
Except I don't have the math chops to do that.  So here's what I did... Turned to my trusty friend Excel, and plotted a poor man's 3d surface.  We already know the relationship between Incentives and Increase in Probability, so we can multiply the latter by each premium amount for a sensible range of Incentive values, and deduct the cost to calculate the ROI (see below).
...
## Performance
Well, I got busy as the deadline approached, and never submitted for a final score.  I had made submissions that were on the top ~1/3 of the leaderboard before figuring out the optimization part, but "the platform" this particular competition was on doesn't automatically submit your best score, so I don't know how I fared on the final leaderboard... But that wasn't my intent anyways.  As I said, very disappointing the problem doesn't stay up for people to use to learn.  A bit anti-climactic I suppose. Lesson learned, to stay away from this platform.  
