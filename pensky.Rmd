...
* Probability ranges from 0-1.  If a customer is predicted to have a high probability of renewing their policy (say ~0.99), you cannot incentivize their policy to a combined probability > 1.  In other words, **the maximum revenue per policy, before subtracting incentives, is the Premium.**   
* You'll want to maximize those policies that already have high baseline P(renewal) to 1, because small Improvements don't cost very much (curve is steepest closer to 0).  That seems silly and probably not something you'd do in real life, BUT the problem did not account for compensation of the agent, so **your only goal is to maximize return according to the formula given.**  
