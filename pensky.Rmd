...
testX <- testX %>%
  ...
  mutate(incen_room = predict(drc2, data.frame(x = actual_delta_P))) %>% 
  # mutate(incen_funmodel = fun.model(actual_delta_P)) %>% 
  mutate(p_incentives = pred + actual_delta_P) %>% 
  mutate(incentives = trunc(pmin(ideal_incentive, incen_room))) %>% # take the lower value
